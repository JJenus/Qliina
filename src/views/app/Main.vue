<script setup>
	...
	function searchDB(limit, offset) {
		...
		$.ajax({
			...
			method: "POST",
			data: { limit: limit, offset: offset, q: inputs.value.search },
			success: (res) => {
				//console.log(res);
				if (offset == 0) {
					customers.value = [];
										...
										>
											...
											>
												<h1 class="fw-bolder text-dark">
													Search for customers ID or
													name
												</h1>
												<div class="fs-3 mb-8">
													ID search is exact while
													name search is relative
												</div>
												<!--begin::Form-->
												<form
													@submit.prevent="
